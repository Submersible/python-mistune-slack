...
from mistune import BlockState, InlineParser, InlineState, Markdown
...
def slack_plugin(md: Markdown, *, no_backtick_relace: bool = False):
    ...
    EMOJI_PATTERN = r"(?P<slack_emoji_raw>(?P<slack_emoji>[a-z0-9_\-]+):)"
    ...
    def escape_code_backtick(md: Markdown, state: BlockState):
        print("---- MD -----")
        pprint(md)
        print("---- STATE -----")
        pprint(state)
    ...
    md.before_parse_hooks.append(escape_code_backtick)
